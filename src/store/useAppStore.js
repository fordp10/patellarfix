...
function computeNewPhase(profile, sessions) {
  ...
  if (profile.currentPhase === 1 && weeksElapsed >= 2 && fullSessions >= 4) return 2
  if (profile.currentPhase === 2 && weeksElapsed >= 8 && fullSessions >= 16) return 3
...
export function useAppStore() {
  ...
  // Check phase advancement once on app load (time may have elapsed since last session)
  useEffect(() => {
    setState(prev => {
      const newPhase = computeNewPhase(prev.profile, prev.sessions)
      if (newPhase === prev.profile.currentPhase) return prev
  ...
  const saveSession = useCallback((session) => {
    setState(prev => {
      ...
      const newPhase = computeNewPhase(prev.profile, newSessions)
      return {
        ...prev,
        sessions: newSessions,
        profile: newPhase !== prev.profile.currentPhase
          ? { ...prev.profile, currentPhase: newPhase }
          : prev.profile,
      }
    })
  }, [])

  const checkPhaseAdvancement = useCallback(() => {
    setState(prev => {
      const newPhase = computeNewPhase(prev.profile, prev.sessions)
      if (newPhase === prev.profile.currentPhase) return prev
      return { ...prev, profile: { ...prev.profile, currentPhase: newPhase } }
